	...
	while True:
		try:
			...
			if cmd[0] == 62: #caractère ">"
				...
				if print_out == 1:
					print('Envoi Statut Cycle= {}'.format(_Statut))
				conn.sendall(_Statut)
				conn.close()
...
def check_fly():
	...
	while True:
		...
		if allow_to_fly == 1:
			allow_to_fly = 0
			# Attendre que le verrou saute
			# while m.verrou == 1:
			#	 time.sleep(1)
			#	 pass
			# Resultat_Execution = Read_Prgm(prgm_planifier)
			# Saute le programme prévu si le verrou n'est pas à zéro
			if m.verrou == 0:
				Resultat_Execution = Read_Prgm(prgm_planifier)
			allow_to_fly = 0
...
#caractère "S"
_Statut = 'S = {} {} {} {} {}'.format(m.PIC_ASK('S'), m.version, ARRET_URGENCE(), m.batterie(), m.wifi_nom)
print(_Statut)
#caractère "P"
_Statut = 'P = {} {} {}'.format(m.Run_Prgm, prgm_planifier, m.verrou)
print(_Statut)
