    ...
    $.ajax({
        ...
        success: function(contenu) {
            ...
            for (var ligne in tab_l) {
                ...
                if (tab_c.length == 4) {
                    var txt_opt = '';
                    if (tab_c[0] == 'True') { var act = 'checked'; } else { var act = ''; }
                    ...
                    for (var s_opt in tab_opt) {
                        ...
                        if (tab_c[3] == tab_opt[s_opt]) { txt_opt += 'selected'; }
...
function save_conf() {
	...
	else if (neg) { alerter(z_c, "Aucune valeure nulle ou négative n'est autorisée", "", ""); } else {
...
function save_edit() {
    ...
    if (valeur != '') {
        ...
        if (val_txt.length > 0) {
            ...
            if (tab.length != tab2.length) { err = 1; }
            for (ev in tab2) {
                if (tab[ev] != tab2[ev]) {
                    erl = tab2[ev];
                    ligne = ev;
            ...
            } else { alerter(ed, "Ligne " + ligne + " : " + erl + " n'est pas reconnu", "", ""); }
