...
#include "define.h"
    ...
    unsigned long long Compteur_Fonctionnement;
    
    unsigned int Batterie_mV;
    //Variable de fonctionnement
    
    unsigned char P4_busy;
    
    unsigned char Pailleux_Valeur;//Paramètre de la fonction DVT: nombre de pion à franchir avant d'effectuer une manoeuvre de recule
...
//#define DVT_TABLEAU_TAILLE 6
    ...
    unsigned char Mouvement_Startup;// activer au lancement de la commande moteur, éteint lors d'un front descendant sur le capteur PION
    ...
    unsigned char nb_s_fonctionnement_souhaiter;
    ...
    unsigned char nb_100ms_platine_backup;
//    unsigned char nb_100ms_platine_fin;
    ...
    volatile char Consigne_Max;
    volatile char Consigne_Max_Moteur_X;
    volatile char Consigne_Max_Moteur_Z;
    volatile char Consigne_Max_Moteur_Bequille;
    
    unsigned char Moteur_I_Max;
    unsigned char Pelle_I_Max;
    unsigned char Bequille_I_Max;
    ...
    unsigned char type_consigne;
    unsigned char mouvement_option;
    ...
    unsigned char Surveillance_Consommation_Moteur_1;
    unsigned char Surveillance_Consommation_Moteur_2;
    
    unsigned char Surveillance_Capteur_Pion;
    
   volatile  unsigned char Compteur_IRQ_I_Moteur_1;
   volatile  unsigned char Compteur_IRQ_I_Moteur_2;
   ...
   volatile bit RX1_ABR; // Variable de l'autobaudrate
   
   
    volatile bit RX1_full;
    volatile bit SPI_full;
...
volatile unsigned char eusart1TxBuffer_Test[10];
volatile bit UART_TEST;
    

volatile bit SPI_MODE;

volatile unsigned char spiTxIndex; //index du buffer de transmission
volatile unsigned char spiTxBuffer[SPI_TX_BUFFER_SIZE]; //Buffer d'émission

volatile unsigned char spiRxIndex; //index du buffer de transmission
volatile unsigned char SPI_RX_TEXT[SPI_RX_BUFFER_SIZE];
volatile unsigned char spiRxBuffer[SPI_RX_BUFFER_SIZE]; //Buffer de réception
